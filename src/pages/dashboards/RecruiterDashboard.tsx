...
const RecruiterDashboard = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "Available": return "bg-brand-accent-green text-white";
      case "In Process": return "bg-brand-secondary text-white";
      case "Interview Stage": return "bg-brand-primary text-white";
      case "Contract Negotiation": return "bg-brand-accent-orange text-white";
      case "Reference Check": return "bg-yellow-500 text-white";
  ...
  return (
    <DashboardLayout role="recruiter" userName="Alexandra Smith" userEmail="alex.smith@recruitment.com">
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            You have 3 urgent client requests and 5 candidates ready for placement.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="font-heading text-xl">Active Placements</CardTitle>
              ...
              <CardContent className="space-y-4">
                {activePlacements.map((placement) => (
                  <div key={placement.id} className="p-4 border border-border rounded-lg hover:border-brand-primary/30 hover:shadow-sm transition-all">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <h3 className="font-semibold text-lg">{placement.position}</h3>
                        <p className="text-muted-foreground">{placement.client}</p>
                        <p className="text-sm text-muted-foreground">Candidate: {placement.candidate}</p>
                      </div>
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="font-heading text-lg">New Client Requests</CardTitle>
                <CardDescription>
                ...
                {clientRequests.slice(0, 3).map((request) => (
                  <div key={request.id} className="p-3 border border-border rounded-lg">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h4 className="font-semibold text-sm">{request.position}</h4>
                        <p className="text-xs text-muted-foreground">{request.school}</p>
                      </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="font-heading text-lg">Top Candidates</CardTitle>
                <CardDescription>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div>
                    <div className="text-xl font-bold text-brand-primary">8</div>
                    <div className="text-xs text-muted-foreground">Successful Placements</div>
                  </div>
                  <div>
                    <div className="text-xl font-bold text-brand-accent-green">94%</div>
